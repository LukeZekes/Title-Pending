...
public class Fire_Controller : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (Input.GetButton("Fire1(Controller)") && shootTimer <= 0)
        {
        ...
        if (Input.GetButtonUp("Fire1(Controller)"))
        {
            ...
            if (chargingCounter > 10)
            {
                ...
                this.transform.GetComponentInParent<PlayerMovement_Controller>().Knockback(knockbackForce);
            }
            else
            {
                this.transform.GetComponentInParent<PlayerMovement_Controller>().Knockback(baseKnockbackForce);
